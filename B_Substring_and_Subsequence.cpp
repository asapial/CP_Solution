...
for(int i=0;i<b.size()-1;i++)
{
    if(b[i]==a[0] and b[i+1]==a[a.size()-1])
    {
        ans-=2;
    }
}

cout<<minl((b.size()+a.size()-maxi),ans)<<endl;
...
int regular_length=total_length-lsc(a,b);

reverse(all(a));
reverse(all(b));

int reverse_length=total_length-lsc(a,b);

cout<<min(reverse_length,regular_length)<<endl;
}
