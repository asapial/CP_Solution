...
int n;
cin>>n;
vector<int>a(n),parity;
cin>>a;
int ans=0;
for(int i=0;i<n;i++)if(a[i]%2==0)parity.push_back(a[i]);

for(int i=0;i<n;i++)if(a[i]%2)parity.push_back(a[i]);

for(int i=0;i<n-1;i++)
{
    for(int j=i+1;j<n;j++)
    {
        if(__gcd(parity[i],2*parity[j])>1)ans++;
    }
}
cout<<ans<<endl;
return ;
